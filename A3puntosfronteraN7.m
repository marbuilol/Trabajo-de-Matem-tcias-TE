...
%Fichero de coordenadas x importado
xx = fscanf(fileID1,'%f');
%Fichero de coordenadas y importado
yy = fscanf(fileID2,'%f');

%Frontera exterior
frontr=boundary(xx,yy,0.9); 

% Obtención de los lados del rectángulo del depósito

in1=find (xx>=0.2 & xx<=1.2 & yy<=1*10^-5 & yy>=-1*10^-5); %lado inferior
...
%Modificación empírica para eliminar índices repetidos y ordenarlos
a=in1(2);
in1(2)=[];
in1(n1)=a;
...
%Representación gráfica del contorno ext e int del mallado
figure(1)
plot(xx,yy,'x')
hold on
...
%Comprobación si se cogen todos los nodos de las fronteras ext e int
figure(2)
plot(xx,yy,'x')
hold on
